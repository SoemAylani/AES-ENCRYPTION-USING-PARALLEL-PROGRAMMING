...
public:
    ...
    static void search( unsigned int offsets[16],
                ...
                uint8_t byte_min,
                uint8_t byte_max,
                uint64_t &loop_cnt,                 // output the number of iterations actually done
                bool &found                         // output
            ){
        uint64_t n_loops = 1;
        ...
        for(uint64_t cnt = 0; cnt < n_loops; cnt++){
            ...
            aes128_load_key_enc_only(key_local, key_schedule);
...
int main (int argc, char*argv[]){
    ...
    switch(usage){
        ...
        case 2:{
            ...
            byte_max = input_bytes[byte_range-1];
